...
export function NewCourseModal({ isAdmin, onClose }: NewCourseModalProps) {
  ...
  const [disciplines, setDisciplines] = useState<{id: string, name: string, syllabus: string, teacherCount: number, isExpanded: boolean}[]>([]);
  ...
  const handleSave = async () => {
    ...
    try {
      const courseDb = mapCourseToDb({
        ...
        enrollmentStatus,
        websiteUrl,
        specificDisciplines: disciplines.map((d: any) => ({ 
          name: d.name, 
          syllabus: d.syllabus || '',
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <div className="flex items-end">
                <Button onClick={() => {
                  if (newDisc) {
                    setDisciplines([...disciplines, { id: `new-${Date.now()}`, name: newDisc, syllabus: '', teacherCount: 1, isExpanded: true }]);
                    setNewDisc('');
                  }
                    ...
                    <SortableItem key={disc.id} id={disc.id}>
                      <div className="flex flex-col gap-2 p-1.5 bg-gray-50 rounded-lg border border-gray-200 pl-8 sm:pl-10 relative group hover:border-indigo-200 transition-all">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center bg-white px-2 py-0.5 rounded border border-gray-100 gap-2 shrink-0">
                            <span className="text-[9px] font-black uppercase text-gray-400">Docentes:</span>
                            <div className="flex items-center gap-1">
                              <button 
                                onClick={() => {
                                  const next = [...disciplines];
                                  next[i] = { ...next[i], teacherCount: Math.max(1, next[i].teacherCount - 1) };
                                  setDisciplines(next);
                                }}
                                className="w-5 h-5 flex items-center justify-center rounded bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold"
                              >
                                -
                              </button>
                              <span className="text-xs font-black min-w-[1ch] text-center">{disc.teacherCount}</span>
                              <button 
                                onClick={() => {
                                  const next = [...disciplines];
                                  next[i] = { ...next[i], teacherCount: Math.min(5, next[i].teacherCount + 1) };
                                  setDisciplines(next);
                                }}
                                className="w-5 h-5 flex items-center justify-center rounded bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold"
                              >
