...
export function HolidaysManager({ holidays, isAdmin }: HolidaysManagerProps) {
  ...
  const handleDeleteYearHolidays = () => {
    const count = holidays.filter((h: any) => h.date.startsWith(`${selectedYear}-`)).length;
    if (count === 0) {
      alert(`Nenhum feriado encontrado para o ano ${selectedYear}.`);
      return;
    }
    setShowYearDeleteConfirm(true);
  };

  const confirmDeleteYearHolidays = async () => {
    setIsDeleting(true);
    try {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingId) {
        ...
        if (form.description.toLowerCase().trim() === 'carnaval') {
          const baseDate = parseISO(form.date);
          const carnavalDays = [
            { offset: -4, desc: 'Carnaval (Sexta)' },
            { offset: -3, desc: 'Carnaval (Sábado)' },
            { offset: -2, desc: 'Carnaval (Domingo)' },
            { offset: -1, desc: 'Carnaval (Segunda)' },
            { offset: 0, desc: 'Carnaval' },
            { offset: 1, desc: 'Quarta-feira de Cinzas' }
          ];

          const existingDates = new Set(holidays.map((h: any) => h.date));
          let expandedCount = 0;

          for (const day of carnavalDays) {
            const currentDate = addDays(baseDate, day.offset);
      ...
      {isAdmin && (
        <Card className="p-4 sm:p-6 border-indigo-100 bg-indigo-50/30 flex flex-col items-center text-center space-y-6">
          ...
          <div className="pt-2 flex flex-wrap justify-center gap-2">
            <Button variant="ghost" onClick={handleRemoveDuplicates} className="text-red-500 hover:bg-red-50 border-red-100 text-[10px] sm:text-xs">
              <Trash2 className="w-3.5 h-3.5 mr-2" /> Limpar Duplicados
            </Button>
            <Button variant="ghost" onClick={handleDeleteYearHolidays} className="text-red-600 hover:bg-red-100 border-red-200 text-[10px] sm:text-xs">
              <Trash2 className="w-3.5 h-3.5 mr-2" /> Esvaziar Ano {selectedYear}
            </Button>
          </div>
        </Card>
      )}
