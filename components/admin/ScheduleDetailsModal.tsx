...
export function ScheduleDetailsModal({ schedule, courses, teachers, isAdmin, onClose, holidays }: ScheduleDetailsModalProps) {
  ...
  const getTeacherCount = (c: any) => {
    if (c.isCommon) return 1;
    const course = courses.find(course => course.id === c.courseId);
    if (!course) return 1;
    const disc = (course.specificDisciplines || []).find((d: any) => (typeof d === 'string' ? d : d.name) === c.disciplineName);
    if (typeof disc === 'object') return disc.teacherCount || 1;
    return 1;
  };
  ...
  const updateClassField = (order: number, field: string, value: any, courseId?: string) => {
            ...
            {isAdmin && isEditing && (
              <>
                <Button 
                  ...
                  className="h-10 px-4 bg-indigo-600/10 hover:bg-indigo-600/20 border-indigo-200 text-indigo-700 text-[10px] sm:text-xs font-black uppercase tracking-widest"
                >
                  <Calendar className="w-4 h-4 mr-2" /> 
                  <span className="hidden xs:inline">Atualizar p/ Feriados</span>
                  <span className="xs:hidden">Holidays</span>
                </Button>
            ...
            {!isEditing && isAdmin && (
              <>
                <Button 
                  ...
                  className="h-10 px-4 bg-indigo-50 hover:bg-indigo-100 border-indigo-200 text-indigo-600 text-[10px] sm:text-xs font-black uppercase tracking-widest"
                >
                  <RotateCcw className="w-4 h-4 mr-2" /> <span className="hidden xs:inline">Reprocessar Carnaval/Feriados</span><span className="xs:hidden">Sync Feriados</span>
                </Button>
                <Button 
                  variant="secondary" 
                  size="sm" 
                          ...
                          <Card className={`overflow-hidden border-none shadow-md break-inside-avoid ${isEditing ? 'ring-2 ring-indigo-100' : ''}`}>
                            <div className={`p-4 flex flex-col md:flex-row md:items-start justify-between gap-6 ${isCommon ? 'bg-indigo-50/50' : 'bg-white'}`}>
                              ...
                              <div className="w-full md:w-80 shrink-0">
                                <p className="text-[10px] font-bold text-gray-400 uppercase mb-3 tracking-wider">Professores Responsáveis</p>
                                <div className="space-y-4 pr-2">
                                  {sortedCourseGroups.map((c: any) => {
                                    const teacherCount = getTeacherCount(c);
                                    return (
                                        ...
                                        {isEditing ? (
                                          <div className="space-y-2 p-3 bg-gray-50/50 rounded-xl border border-gray-200">
                                            <p className="text-[9px] font-bold text-gray-400 uppercase tracking-tight">
                                              {c.isCommon ? 'Fase Comum' : c.courseName?.split(' ')[0]} 
                                              {teacherCount > 1 && <span className="ml-2 text-indigo-600">({teacherCount} docentes)</span>}
                                            </p>
                                            
                                            {Array.from({ length: teacherCount }).map((_, tIdx) => (
                                              <Select 
                                                key={tIdx}
                                                value={c.teacherIds?.[tIdx] || ''} 
                                                onChange={(e: any) => {
                                                  const newIds = [...(c.teacherIds || [])];
                                                  while (newIds.length < teacherCount) newIds.push('');
                                                  newIds[tIdx] = e.target.value;
                                                  updateClassField(order, 'teacherIds', newIds, c.isCommon ? undefined : c.courseId);
                                                }}
                                                className="h-9 text-xs bg-white mb-2"
                                              >
                                                <option value="">{teacherCount > 1 ? `Selecione o Professor ${tIdx + 1}` : 'Selecione um Professor'}</option>
                                                {(teachers || [])
                                                  .sort((a: any, b: any) => {
                                          ...
                                          <div className="space-y-2">
                                            {(c.teacherIds || []).filter(Boolean).map((tid: string, idx: number) => {
                                            ...
                                            {(!c.teacherIds || c.teacherIds.length === 0) && (
                                              <div className="flex items-center gap-3 p-3 bg-white rounded-xl border border-gray-100 shadow-sm opacity-50 italic">
                                                <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 bg-gray-100 text-gray-400">?</div>
                                                <div className="flex flex-col min-w-0">
                                                  <p className="text-[9px] text-gray-400 font-bold uppercase leading-none mb-1">{c.isCommon ? 'Fase Comum' : c.courseName}</p>
                                                  <p className="text-xs font-semibold text-gray-600 truncate">Não atribuído</p>
                                                </div>
                                              </div>
                                            )}
