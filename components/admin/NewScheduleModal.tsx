...
export function NewScheduleModal({ courses, holidays, teachers, commonDisciplines, onClose }: NewScheduleModalProps) {
  ...
  const getTeacherCount = (disciplineName: string, courseId?: string) => {
    if (!courseId) return 1; // Fase Comum sempre 1 docente por enquanto
    const course = courses.find(c => c.id === courseId);
    if (!course) return 1;
    const disc = (course.specificDisciplines || []).find((d: any) => (typeof d === 'string' ? d : d.name) === disciplineName);
  ...
  const generatePreview = () => {
    ...
    const initializedCommon = (result.common || []).map((c: any) => {
      ...
      return { ...c, teacherId: matchingTeacher?.id || '', teacherIds: matchingTeacher?.id ? [matchingTeacher.id] : [] };
    });
    ...
    for (const courseId in result.specific) {
      initializedSpecific[courseId] = (result.specific[courseId] || []).map((c: any) => {
        const teacherCount = getTeacherCount(c.disciplineName, courseId);
        // Pre-select teacher if they are registered for this discipline
        const matchingTeacher = teachers.find((t: any) => 
          t.specialties?.some((s: any) => s.disciplineName === c.disciplineName)
        );
        const teacherIds = matchingTeacher?.id ? [matchingTeacher.id] : [];
        while (teacherIds.length < teacherCount) teacherIds.push('');
        
        return { ...c, teacherId: matchingTeacher?.id || '', teacherIds };
      });
    }
                    ...
                    return (
                      <Card key={order} className="p-4 space-y-3">
                        ...
                        <div className="space-y-3">
                          {Array.from({ length: getTeacherCount(firstClass.disciplineName) }).map((_, tIdx) => (
                            <Select 
                              key={tIdx}
                              label={getTeacherCount(firstClass.disciplineName) > 1 ? `Docente ${tIdx + 1}` : "Docente"}
                              options={(teachers || [])
                                .sort((a: any, b: any) => {
                              ...
                              value={firstClass.teacherIds?.[tIdx] || ''} 
                              onChange={(e: any) => {
                                const newCommon = preview.common.map((c: any) => {
                                  if (c.order === order) {
                                    const newIds = [...(c.teacherIds || [])];
                                    while (newIds.length <= tIdx) newIds.push('');
                                    newIds[tIdx] = e.target.value;
                                    return { ...c, teacherIds: newIds };
                                  }
                                  return c;
                                });
                      ...
                      {Array.from(new Set(preview.specific[courseId].map((c: any) => c.order))).sort((a: any, b: any) => a - b).map((order: any) => {
                        ...
                        return (
                          <Card key={order} className="p-4 space-y-3">
                            ...
                            <div className="space-y-3">
                              {Array.from({ length: getTeacherCount(firstClass.disciplineName, courseId) }).map((_, tIdx) => (
                                <Select 
                                  key={tIdx}
                                  label={getTeacherCount(firstClass.disciplineName, courseId) > 1 ? `Docente ${tIdx + 1}` : "Docente"}
                                  options={(teachers || [])
                                    .sort((a: any, b: any) => {
                                      const aIsSpec = a.specialties?.some((s: any) => s.disciplineName === firstClass.disciplineName);
                                      const bIsSpec = b.specialties?.some((s: any) => s.disciplineName === firstClass.disciplineName);
