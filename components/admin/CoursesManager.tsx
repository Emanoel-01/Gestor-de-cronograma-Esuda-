...
function CourseEditModal({ course, isAdmin, onClose }: any) {
  ...
  const updateDiscTeacherCount = (index: number, count: number) => {
    const next = [...disciplines];
    next[index] = { ...next[index], teacherCount: Math.max(1, Math.min(5, count)) };
    setDisciplines(next);
  };
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="flex items-center bg-white px-2 py-0.5 rounded border border-gray-100 gap-2 shrink-0">
                            ...
                            <div className="flex items-center gap-1">
                              <button 
                                onClick={() => updateDiscTeacherCount(i, disc.teacherCount - 1)}
